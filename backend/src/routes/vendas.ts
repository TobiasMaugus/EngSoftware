...
import { authenticateToken, verifyDeletePassword, AuthRequest } from '../middleware/auth.js';
...
router.get('/', authenticateToken, async (req: AuthRequest, res) => {
  try {
    ...
    const [total]: any = await pool.execute(
      'SELECT COUNT(*) as count FROM vendas WHERE usuario_id = ?',
      [req.user.userId]
    );
...
router.post('/', authenticateToken, async (req: AuthRequest, res) => {
  ...
  try {
    ...
    const [vendaResult]: any = await connection.execute(
      'INSERT INTO vendas (cliente_id, usuario_id, total) VALUES (?, ?, ?)',
      [cliente_id, req.user.userId, total]
    );
    ...
    for (const item of itens) {
      ...
      await connection.execute(
        ...
        [vendaId, item.produto_id, item.quantidade, produto[0].preco, subtotal]
      );
...
router.delete('/:id', authenticateToken, verifyDeletePassword, async (req: AuthRequest, res) => {
  const connection = await pool.getConnection();
  ...
  try {
    ...
    const [venda]: any = await connection.execute(
      'SELECT * FROM vendas WHERE id = ? AND usuario_id = ?',
      [id, req.user.userId]
    );
...
router.get('/periodo', authenticateToken, async (req: AuthRequest, res) => {
  try {
    ...
    const [vendas] = await pool.execute(
      `SELECT v.*, c.nome as cliente_nome 
       ...
       ORDER BY v.data_venda DESC 
       LIMIT ? OFFSET ?`,
      [req.user.userId, dataInicio, dataFim, limit, offset]
    );
    
    const [total] = await pool.execute(
      'SELECT COUNT(*) as count FROM vendas WHERE usuario_id = ? AND data_venda BETWEEN ? AND ?',
      [req.user.userId, dataInicio, dataFim]
    );
