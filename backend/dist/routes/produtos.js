...
router.get('/', authenticateToken, async (req, res) => {
    try {
        ...
        const [produtos] = await pool.execute(query, [...params, limit, offset]);
...
router.get('/buscar/:nome', authenticateToken, async (req, res) => {
    try {
        ...
        const [produtos] = await pool.execute('SELECT id, nome, preco, estoque FROM produtos WHERE nome LIKE ? AND estoque > 0 LIMIT 10', [`%${nome}%`]);
        res.json(produtos);
    }
    catch (error) {
        res.status(500).json({ error: 'Erro ao buscar produtos' });
...
router.patch('/:id/estoque', authenticateToken, async (req, res) => {
    try {
        ...
        // Atualizar estoque
        await pool.execute('UPDATE produtos SET estoque = estoque + ? WHERE id = ?', [quantidade, id]);
...
router.get('/estatisticas/geral', authenticateToken, async (req, res) => {
    try {
      ...
      FROM produtos
    `);
        const [baixoEstoque] = await pool.execute('SELECT COUNT(*) as produtos_baixo_estoque FROM produtos WHERE estoque < 10');
        res.json({
            ...estatisticas[0],
            ...baixoEstoque[0]
        });
