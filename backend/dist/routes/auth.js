...
router.post('/google', async (req, res) => {
    try {
        ...
        const [users] = await pool.execute('SELECT * FROM usuarios WHERE google_id = ? OR email = ?', [googleId, email]);
        let userId;
        if (users.length > 0) {
            // Atualizar usuário existente
            userId = users[0].id;
            await pool.execute('UPDATE usuarios SET nome = ?, google_id = ? WHERE id = ?', [name, googleId, userId]);
        }
        else {
            // Criar novo usuário
            const [result] = await pool.execute('INSERT INTO usuarios (google_id, nome, email) VALUES (?, ?, ?)', [googleId, name, email]);
